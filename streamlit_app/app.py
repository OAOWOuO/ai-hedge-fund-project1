...
st.caption("v6.0 | Portfolio Allocator & Stock Analyzer | Yahoo Finance (15-20 min delayed)")

# ============== TABS ==============
tab_analyzer, tab_signals, tab_portfolio, tab_analytics, tab_performance, tab_events, tab_trades, tab_analysts, tab_securities, tab_settings = st.tabs([
    "🔬 Stock Analyzer", "📈 Signals", "💼 Portfolio", "📊 Analytics", "🏆 Performance", "📅 Events & Income", "📋 Trades", "🧠 Analysts", "🔍 Securities", "⚙️ Settings"
])
...
with tab_analyzer:
    ...
    st.caption("Deep-dive technical and fundamental analysis with AI recommendations")

    col_input, col_analysis = st.columns([1, 3])

    # Data functions for analyzer
    @st.cache_data(ttl=300)
    ...
    def get_recommendation(data, tech_df):
        ...
        if len(tech_df) > 50:
            ...
            if latest['RSI'] < 30:
                score += 2
                signals.append(("RSI Oversold", "bullish", f"RSI at {latest['RSI']:.0f}"))
            elif latest['RSI'] > 70:
                score -= 2
                signals.append(("RSI Overbought", "bearish", f"RSI at {latest['RSI']:.0f}"))
            else:
            ...
            if latest['MACD'] > latest['MACD_Signal']:
                score += 1
                signals.append(("MACD Bullish", "bullish", "Above signal line"))
            else:
                score -= 1
                signals.append(("MACD Bearish", "bearish", "Below signal line"))
            # MA
            max_score += 2
            if pd.notna(latest['MA50']) and pd.notna(latest['MA200']):
                if latest['MA50'] > latest['MA200']:
                    score += 1
                    signals.append(("Golden Cross", "bullish", "MA50 > MA200"))
                else:
                    score -= 1
                    signals.append(("Death Cross", "bearish", "MA50 < MA200"))
        # Fundamentals
        ...
        if pe:
            ...
            if pe < 15:
                score += 2
                signals.append(("Low P/E", "bullish", f"P/E {pe:.1f}"))
            elif pe > 35:
                score -= 2
                signals.append(("High P/E", "bearish", f"P/E {pe:.1f}"))
        ...
        if target and price:
            ...
            if upside > 15:
                score += 2
                signals.append(("Strong Upside", "bullish", f"{upside:.0f}% to target"))
            elif upside < -10:
                score -= 2
                signals.append(("Downside Risk", "bearish", f"{upside:.0f}% to target"))
    ...
    with col_input:
        ...
        analyzer_ticker = st.text_input("Ticker", value="AAPL", key="analyzer_ticker", label_visibility="collapsed")
        analyzer_ticker = analyzer_ticker.strip().upper()
        ...
        if analyze_btn and analyzer_ticker:
            with st.spinner(f"Analyzing {analyzer_ticker}..."):
                st.session_state.analyzer_data = fetch_stock_analysis(analyzer_ticker)
        ...
        if st.session_state.analyzer_data and st.session_state.analyzer_data.get("valid"):
            ...
            if d['high_52w'] > d['low_52w']:
                ...
                st.progress(min(1, max(0, pos)), text=f"Range: {pos*100:.0f}%")
        ...
        if st.session_state.analyzer_data and st.session_state.analyzer_data.get("valid"):
            ...
            if hist is not None and not hist.empty:
                ...
                with tab_tech:
                    ...
                    with col_rsi:
                        ...
                        rsi_chart = alt.Chart(chart_data.tail(100)).mark_line(color='#58a6ff').encode(
                            x=alt.X('Date:T', title=None), y=alt.Y('RSI:Q', scale=alt.Scale(domain=[0,100]))
                        )
                    ...
                    with col_macd:
                        st.write("### MACD")
                        macd_chart = alt.Chart(chart_data.tail(100)).mark_line(color='#58a6ff').encode(x='Date:T', y='MACD:Q')
                        ...
                        if tech_df['MACD'].iloc[-1] > tech_df['MACD_Signal'].iloc[-1]: st.success("MACD Bullish")
                        else: st.warning("MACD Bearish")
                ...
                with tab_fund:
                    ...
                    c1.metric("P/E", f"{d.get('pe_ratio', 0):.1f}" if d.get('pe_ratio') else "N/A")
                    c2.metric("Forward P/E", f"{d.get('forward_pe', 0):.1f}" if d.get('forward_pe') else "N/A")
                    c3.metric("P/B", f"{d.get('pb_ratio', 0):.1f}" if d.get('pb_ratio') else "N/A")
                    c4.metric("P/S", f"{d.get('ps_ratio', 0):.1f}" if d.get('ps_ratio') else "N/A")

                    st.write("### Growth & Profitability")
                    c1, c2, c3, c4 = st.columns(4)
                    c1.metric("Revenue Growth", f"{d.get('revenue_growth', 0)*100:.1f}%" if d.get('revenue_growth') else "N/A")
                    c2.metric("Earnings Growth", f"{d.get('earnings_growth', 0)*100:.1f}%" if d.get('earnings_growth') else "N/A")
                    c3.metric("Profit Margin", f"{d.get('profit_margin', 0)*100:.1f}%" if d.get('profit_margin') else "N/A")
                    c4.metric("ROE", f"{d.get('roe', 0)*100:.1f}%" if d.get('roe') else "N/A")

                    st.write("### Analyst Targets")
                    if d.get('target_price'):
                        c1, c2, c3, c4 = st.columns(4)
                        c1.metric("Target Low", f"${d.get('target_low', 0):.0f}")
                        c2.metric("Target Mean", f"${d.get('target_price', 0):.0f}")
                        c3.metric("Target High", f"${d.get('target_high', 0):.0f}")
                        upside = (d['target_price'] - d['price']) / d['price'] * 100
                        c4.metric("Upside", f"{upside:+.1f}%")
                        st.caption(f"Based on {d.get('num_analysts', 0)} analysts")
                    else:
                        st.info("No analyst targets available")
        else:
            st.info("👈 Enter a ticker and click **Analyze Stock** to see technical and fundamental analysis with AI recommendations.")
